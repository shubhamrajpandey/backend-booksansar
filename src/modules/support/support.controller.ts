...
const getAdminResolutionEmail = (
  ...
  return {
    ...
    html: emailWrapper(
      isResolved ? "Request Resolved ✓" : "Request Rejected",
      body,
    ),
  };
};
...
export const updateSupportRequest = async (req: Request, res: Response) => {
  try {
    ...
    const support = await Support.findByIdAndUpdate(req.params.id, update, {
      new: true,
    });
    if (!support) return res.status(404).json({ message: "Not found" });

    if (status === "resolved" || status === "rejected") {
      const { subject, html } = getAdminResolutionEmail(
        support.name || "",
        status,
        adminNote,
      );
      await sendEmail(support.email, subject, "", html);
    }

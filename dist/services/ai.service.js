...
exports.aiSearch = exports.chatWithBookSansarAI = exports.getTrendingBooks = exports.getSmartRecommendations = void 0;
const groq_sdk_1 = __importDefault(require("groq-sdk"));
const book_model_1 = __importDefault(require("../modules/book/book.model"));
const order_model_1 = require("../modules/order/order.model");
const user_model_1 = __importDefault(require("../modules/user/user.model"));
const groq = new groq_sdk_1.default({ apiKey: process.env.GROQ_API_KEY });
const MODEL = "llama-3.1-8b-instant";
const getSmartRecommendations = async ({ userId, type = "all", genre, limit = 10, }) => {
    try {
        const filter = { status: "approved" };
        if (type === "free")
            filter.type = "free";
        if (type === "paid")
            filter.type = { $in: ["physical", "second-hand"] };
        if (genre)
            filter.genre = { $regex: genre, $options: "i" };
        const allBooks = await book_model_1.default.find(filter)
            .select("title author category genre description price rating type")
            .limit(100)
            .lean();
        const userOrders = await order_model_1.Order.find({ customerId: userId, status: "delivered" })
            .select("items").lean();
        const purchasedTitles = userOrders.flatMap((o) => o.items.map((i) => i.title)).join(", ") ||
            "No history yet";
        const bookContext = allBooks
            .map((b) => `Title: ${b.title} | Author: ${b.author} | Category: ${b.category} | Rating: ${b.rating || 0}/5 | Price: Rs ${b.price || "Free"} | Type: ${b.type}`).join("\n");
...
const getTrendingBooks = async (type = "all", limit = 10) => {
    const filter = { status: "approved" };
    if (type === "free")
        filter.type = "free";
    if (type === "paid")
        filter.type = { $in: ["physical", "second-hand"] };
    return await book_model_1.default.find(filter)
...
const chatWithBookSansarAI = async (userMessage, conversationHistory = [], userId) => {
    try {
        ...
        const [relevantBooks, topBooks] = await Promise.all([
            book_model_1.default.find({
                ...
                .limit(15)
                .lean(),
            book_model_1.default.find({ status: "approved" })
                .sort({ rating: -1 })
        ...
        if (userId) {
            const [user, orders] = await Promise.all([
                user_model_1.default.findById(userId).select("name").lean(),
            ...
            const allOrderedBooks = orders
                .flatMap((o) => o.items.map((i) => `${i.title} (${o.status})`));
            userContext = `
...
Total orders: ${orders.length}`.trim();
        ...
        const relevantContext = relevantBooks.length > 0
            ? relevantBooks
                .map((b) => `- "${b.title}" by ${b.author} | ${b.category} | ${b.rating || 0}/5 stars | ${b.price ? `Rs ${b.price}` : "FREE"} | ${b.type}`)
                .join("\n")
            : "No specific matches in catalog for this query.";
        const topContext = topBooks
            .map((b) => `- "${b.title}" by ${b.author} | ${b.category} | ${b.rating || 0}/5 | ${b.price ? `Rs ${b.price}` : "FREE"}`)
            .join("\n");
        const messages = [
            {
                role: "system",
...
exports.chatWithBookSansarAI = chatWithBookSansarAI;
const aiSearch = async (query) => {
    try {
        const allBooks = await book_model_1.default.find({ status: "approved" })
            .select("title author category genre description price rating type _id")
            .limit(150)
            .lean();
        const bookList = allBooks
            .map((b) => `ID:${b._id}|${b.title} by ${b.author}|${b.category}|${b.genre || ""}`)
            .join("\n");
        const response = await groq.chat.completions.create({
            model: MODEL,
            max_tokens: 300,
        ...
        return await book_model_1.default.find({ _id: { $in: ids }, status: "approved" }).lean();
    }
    catch {
        return [];
    }
